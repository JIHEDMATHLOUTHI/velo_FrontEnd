...
export class EventService {
  private baseUrl = 'http://localhost:8081/api/event';
...
return this.http.get<event1>("http://localhost:8081/api/event/"+pid, { headers })
}
...
return this.http.get<event1[]>("http://localhost:8081/api/event/getall", { headers })
}
...
clearRating(productId: number): void {
 ...
 getevnts() {
  ...
  public addFile(formData:FormData,id:any): Observable<any>{
    ...
    return this.http.post(`http://localhost:8081/api/event/AddeventFile/${id}`,formData, {  reportProgress :true, observe:'events'});
  }
  public retrieveFile(id:any):Observable<Blob>{
    
    const headers = new HttpHeaders().set('Accept', 'application/octet-stream');
    return this.http.get(`http://localhost:8081/api/event/retrieveFile/${id}`,
      { headers : headers,
               responseType: 'blob'
        });
  }
